...
# ╔═╡ 0d7b9668-5c30-4b03-831b-f2dd61984e6a
begin
	using Plots
	using PlotlyJS
	using Flux
...
# ╔═╡ e4ea2f8b-2308-47ec-a5f1-735c0b3eba9d
Test = [Test_0;Test_90;Test_180;Test_270]

# ╔═╡ a540897b-edf5-4a11-b294-62d263e41db6


# ╔═╡ e2c20117-7af6-44ac-bed8-00db696dfb37


# ╔═╡ 8ff394c6-324c-49f2-8580-19c737a99966
function scale(x,in_min,in_max,out_min,out_max)
	x .* (out_max - out_min) ./ (in_max - in_min) .+ out_min
end

# ╔═╡ 9dfec8cc-4fa0-4f31-bb4c-84b81d3c904e
function atanfp(vi,vq)
	...
	quadrant = (x,y) -> (x .< 0) ? (y .<= 0 ) : (y .>= 0)
	# atan2 = (x,y) -> (x .== 0) ? (y == 0 ? 0 : x ./ y) : ( y ./ x )
	atan2 = (x,y) -> (x .== 0) ? (y == 0 ? 0 : atan(x,y)) : ( atan(y,x))
	result = map((a,i,q) -> a ? atan2(i,q) : atan2(q,i),quadrant.(p_i,p_q),a_i,a_q)
	# result .* 180 ./ pi
	# a_q ./ a_i
end

# ╔═╡ 3dadddec-c53b-4d8f-ac66-adf954925071
function atanfp_norm(vi,vq)
	polarity_bit 	= 0b100000
	amplitude_bit 	= 0b011111
	polar = x -> x ? 1 : -1
	p_i = polar.( (vi .& polarity_bit) .== polarity_bit )
	...
	quadrant = (x,y) -> (x .< 0) ? (y .<= 0 ) : (y .>= 0)
	# atan2 = (x,y) -> (x .== 0) ? (y == 0 ? 0 : x ./ y) : ( y ./ x )
	atan2 = (x,y) -> (x .== 0) ? (y == 0 ? 0 : scaleQI(atan(x,y))) : ( scaleIQ(atan(y,x)))
	result = map((a,i,q) -> a ? atan2(i,q) : atan2(q,i),quadrant.(p_i,p_q),a_i,a_q)
	# result .* 180 ./ pi
	# a_q ./ a_i
end

# ╔═╡ c373071c-fa73-40e0-b055-cce14800587f
function loss(ŷ,y)
	diff =  abs(y .- ŷ)
	if ŷ .< y
		if diff < 1
...
function closest(value,list)
	return map(x -> argmin(map(y->loss.(x ,y), list)),value)
end
...
# ╔═╡ 5e304282-70b1-413f-a0dc-80c8c3a4276f
V = VA .* VP
...
# ╔═╡ dbbaaf31-4d4e-462c-9ad2-05c282d503bf
polarity = polar.(((Test[:,2:3] .|>UInt8) .& polarity_bit) .== polarity_bit) .|> Float64

# ╔═╡ 2177766e-d321-4877-a59a-4032800fe4b9
amplitude =  ((Test[:,2:3] .|>UInt8) .& amplitude_bit) .|> Float64

# ╔═╡ 7a27a825-acb9-46f5-af15-331865dd2cf1
amplitude[98:128,:]

# ╔═╡ 0e50ede4-62a8-4ea4-b735-3a4e5888a10d
value = amplitude .* polarity

# ╔═╡ 943a2b9c-f314-42c3-8fed-420619c35a50
magnitude = sqrt.(sum(value.^2;dims=2))

# ╔═╡ bf7f9258-3494-408f-9951-09e30ac466bc
v_m = value ./ magnitude

# ╔═╡ 33250ae0-6784-4070-8acd-24171cd6906e
phase_rad = (atan.(v_m[:,2],v_m[:,1]) .+ 2*pi) .% (2*pi)

# ╔═╡ f0b3762b-bdbf-4a14-878b-c8e39cc536ad
u,_,v = svd(v_m)

# ╔═╡ f9380aeb-3c86-4110-8b63-9567e7e37746
v_n = v_m 

# ╔═╡ a0a140fd-3d31-455d-bf6f-5e67b1318342
Plots.plot(v_n[:,1],v_n[:,2])

# ╔═╡ 9a8fb145-7913-4691-b63b-33a2157fddc2
Plots.plot((0:127) * 2.8125 , v_m   )

# ╔═╡ 7f93e9e6-d02e-48f0-abbf-d2e0e3111c14
Plots.plot(v_m[:,1],v_m[:,2])

# ╔═╡ fe5efc04-aed6-46d2-9715-674ba1a90349
Plots.plot(v_m[:,1]./v_m[:,2])

# ╔═╡ 7abc3036-dbba-422f-90a8-3441310ba021
begin

	Plots.plot((0:1:127) .* 2.8125,v_m[:,2]./v_m[:,1])
end

# ╔═╡ bda56696-b390-4da3-b1c8-e017073fa1b4
Plots.plot(v_m[:,2]./v_m[:,1] ,(0:1:127) .* 2.8125)

# ╔═╡ 86fb4e4f-4522-4cb7-ab48-69d034286612
Plots.plot((0:1:127) .* 2.8125,(atan.(v_m[:,2],v_m[:,1]) * 180 / pi .+ 360) .% 360 )

# ╔═╡ b9438d39-6d48-4771-ac32-f463c465f459
sa = value[:,2] ./ magnitude

# ╔═╡ 57702601-af2a-4e1d-8e8a-583da830e22b
ca= value[:,1] ./ magnitude

# ╔═╡ 30678a9f-aac0-4350-bdbc-0720711fc903
Plots.plot(value[:,1],value[:,2])

# ╔═╡ 08ebfe91-f766-4ad1-8bcc-619f192ded1a
Plots.plot( value[:,2]./value[:,1],(atan.(value[:,2],value[:,1]) * 180 ./ pi  .+ 360) .% 360)

# ╔═╡ 84520976-2c7b-441f-bb64-2f0f82729aeb
Plots.plot( value[:,2]./value[:,1],Test[:,1])

# ╔═╡ 683754b4-35f1-45be-9e0c-710cb041624f
offset_phase = x -> scale.(x,0,(atan.(value[end,2],value[end,1]) .+ (2 * pi)) .% (2*pi),Test[1,1],Test[end,1]) 

# ╔═╡ e15129ae-2dc5-441a-993c-69c4bae846c1
begin
	t = (atan.(value[:,2],value[:,1]) .+ (2 * pi)) .% (2*pi)
	atan_angle = (atan.(value[:,2],value[:,1]) * 180 ./ pi  .+ 360) .% 360
	
	Plots.plot(t,offset_phase(t))
	
	Plots.plot!( t,Test[:,1])

	Plots.plot!( t,offset_phase(t)-Test[:,1])
	
end

# ╔═╡ ba3ae1f0-026a-47d1-92bf-f6f9dbe60842
Plots.plot(offset_phase(t)-Test[:,1])

# ╔═╡ 0ec398bd-8b9b-45a4-a259-50cae7172139
Plots.plot(offset_phase(atan.(value[:,2],value[:,1])))
...
#polarity
	...
	angle = (atan.(a_q .* p_q, a_i .* p_i) .+ 2*pi) .% (2*pi)
	return ANGLE[closest(angle,phase_rad)]
	
end

# ╔═╡ 319c51f8-fd36-44ee-9815-c90718d26b8f
Plots.plot(cal_phase.(Test[:,2] .|>UInt8,Test[:,3].|>UInt8))

# ╔═╡ b546944f-d771-47f3-bfb1-3f091f9b4e8e


# ╔═╡ bc90f18c-2b77-43ec-a81d-97318eeb2d81
((atand.(polarity[:,2],polarity[:,1]) .- 45 .+ 360) .% 360 )[33]

# ╔═╡ de8faa5b-ba3d-4a96-9983-183889740806
Test[33,1]
...
manifest_format = "2.0"
project_hash = "4a9da40e90dcefdcf64c58e75b012b8c83f97581"
